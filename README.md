...
> 배경·데이터 소스 명세는 `docs/PROJECT_CONTEXT.md`, 현재 작업 상태는 `docs/인수인계.md`를 먼저 읽으세요.
> 서버에 올려 돌리는 방법은 `docs/배포와운영.md`에 있습니다.
...
│
├─ docs\                문서
│  ├─ PROJECT_CONTEXT.md   프로젝트 배경·데이터 소스·앞으로 할 일
│  ├─ 인수인계.md           현재 상태와 다음 작업 (새 세션은 여기부터)
│  ├─ 배포와운영.md        GCP 배포 절차·운영·문제 대응 (용어 설명 포함)
│  └─ 작업요약_1주차.md
│
...
인천교통정보센터(fitic) CCTV **21대**에서 20분 간격으로 프레임 1장씩 캡처합니다.
...
`cameras.toml`의 21대가 이 도구로 추려낸 결과입니다.
...
고농도 구간 데이터가 부족하다는 구조적 리스크(`docs/인수인계.md` §5)의 근거입니다.
